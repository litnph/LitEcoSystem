...
export function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!result.ok) { setError(result.error ?? 'Đăng nhập thất bại.'); setLoading(false) }
  }
    ...
    <div className="flex min-h-screen bg-[#F9F6F2]">
      <div className="hidden flex-col justify-between border-r border-[#E4D9CE] bg-[#F3EEE7] p-12 lg:flex lg:w-1/2">
        ...
        <div>
          ...
          <p className="mt-4 max-w-sm text-base leading-relaxed text-[#6B5B48]">
            Theo dõi thu chi, quản lý trả góp, nợ vay và kỳ sao kê thẻ tín dụng — tất cả trong một nơi.
          </p>
          <ul className="mt-8 space-y-3">
            {['Theo dõi thu / chi theo danh mục', 'Quản lý kỳ sao kê thẻ BNPL', 'Ghi nhận trả góp & nợ vay', 'Chi hộ & thu hồi công nợ'].map((feat) => (
              <li key={feat} className="flex items-center gap-3 text-sm text-[#5C4E3C]">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#EDE6DC] text-[#7A5E3E]">✓</span>
                {feat}
              </li>
            ))}
          </ul>
      ...
      <div className="flex flex-1 items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="form-field">
                ...
                <input type="text" value={username} onChange={(e) => { setUsername(e.target.value); setError(null) }}
                  placeholder="admin hoặc demo" autoComplete="username" autoFocus className="form-input" />
              </div>
              ...
              <div className="form-field">
                ...
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} value={password}
                    ...
                    placeholder="••••••••" autoComplete="current-password" className="form-input pr-12" />
                  <button type="button" onClick={() => setShowPassword((v) => !v)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#BFB0A0] transition hover:text-[#3E3025]" tabIndex={-1}>
                    {showPassword ? (
          ...
          <p className="mt-4 text-center text-xs text-[#9E8E7C]">Dữ liệu được lưu cục bộ trên trình duyệt của bạn.</p>
        </div>
      </div>
    </div>
  )
