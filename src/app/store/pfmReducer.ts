...
import { buildBnplProvidersFromConfig } from './pfmState'
import type {
  BnplProvider,
  MonthlyClose,
  SpendingPeriod,
...
export type PfmAction =
  ...
  | { type: 'SET_CATEGORY_KIND'; payload: { category: string; kind: 'income' | 'expense' } }
  // Master data
  ...
  | { type: 'SET_PAYMENT_SOURCE_INSTALLMENT_LIMIT'; payload: { source: string; limit: number | null } }
  | { type: 'SET_CALENDAR_RULES'; payload: CalendarRules }
  | { type: 'SET_INSTALLMENT_MIN_AMOUNT'; payload: number }
  ...
  | {
      ...
      payload: {
        ...
        installmentPlans?: import('../../entities').InstallmentPlan[]
        debts?: import('../../entities').DebtRecord[]
        receivables?: import('../../entities').ReceivableRecord[]
        confirmedStatements?: import('../../entities').ConfirmedStatement[]
        categories?: string[]
...
export function pfmReducer(state: PfmState, action: PfmAction): PfmState {
  switch (action.type) {
    ...
    case 'CREATE_INSTALLMENT': {
      const { transactionId, tenorMonths, conversionFee, statementDay, firstBillingPeriod } = action.payload
      ...
      const computedFirstBillingPeriod = firstBillingPeriod ?? nextPeriod(
      ...
      const plan: InstallmentPlan = {
        ...
        firstBillingPeriod: computedFirstBillingPeriod,
        paidMonths: 0,
        status: 'active',
      }
    ...
    case 'ADD_RECEIVABLE_COLLECTION': {
      const { receivableId, amount, occurredAt, source, channel, merchant, note } = action.payload
      const target = state.receivables.find((r) => r.id === receivableId)
      if (!target) return state
      const isLoanGiven = target.kind === 'loan_given'
      const incomeTx: Transaction = {
        id: newId('tx'),
        type: 'income',
        amount,
        occurredAt,
        merchant: merchant?.trim() || (isLoanGiven ? `Thu nợ: ${target.borrower}` : `Hoàn tiền: ${target.borrower}`),
        channel,
        category: isLoanGiven ? 'Tài chính/Thu hồi công nợ' : 'Tài chính/Hoàn tiền',
        source,
        paymentMode: 'direct',
        businessType: 'receivable_collection',
        counterpartyName: target.borrower,
        relatedReceivableId: target.id,
        note: note?.trim() || `${isLoanGiven ? 'Thu hồi công nợ' : 'Thu hồi khoản chi hộ'} "${target.title}"`,
      }
      return {
        ...state,
        ...
        receivables: state.receivables.map((r) => {
          ...
          const collectedAmount = r.collectedAmount + amount
          return {
            ...r,
            collectedAmount,
            status: collectedAmount >= r.principal ? ('settled' as const) : r.status,
          }
        }),
      }
    }
    ...
    case 'SET_CATEGORY_KIND': {
      const { category, kind } = action.payload
      if (!state.categories.includes(category)) return state
      return {
        ...state,
        categoryKinds: { ...state.categoryKinds, [category]: kind },
      }
    }
    ...
    case 'ADD_MASTER_ITEM': {
      ...
      if (list === 'categories') {
        nextState.categoryKinds = {
          ...state.categoryKinds,
          [nextValue]: state.categoryKinds[nextValue] ?? 'expense',
        }
      }
      if (list === 'paymentSources') {
        ...
        nextState.paymentSourceInstallmentLimits = {
          ...(state.paymentSourceInstallmentLimits ?? {}),
          [nextValue]: (state.paymentSourceInstallmentLimits ?? {})[nextValue] ?? null,
        }
      }
      return nextState
    }
    ...
    case 'REMOVE_MASTER_ITEM': {
      ...
      if (list === 'paymentSources') {
        ...
        const installmentLimits = { ...(state.paymentSourceInstallmentLimits ?? {}) }
        delete installmentLimits[value]
        nextState.paymentSourceInstallmentLimits = installmentLimits
        nextState.bnplProviders = state.bnplProviders.filter((p) => p.name !== value)
      }
      if (list === 'categories') {
        const kinds = { ...state.categoryKinds }
        delete kinds[value]
        nextState.categoryKinds = kinds
      }
      return nextState
    }
    ...
    case 'RENAME_MASTER_ITEM': {
      ...
      if (list === 'categories') {
        const kinds = { ...state.categoryKinds }
        const kind = kinds[oldVal] ?? 'expense'
        delete kinds[oldVal]
        kinds[nextVal] = kind
        nextState.categoryKinds = kinds
      }
      if (list === 'paymentSources') {
        ...
        nextState.paymentSourceDueDays = dueDays
        const oldLimit = (state.paymentSourceInstallmentLimits ?? {})[oldVal] ?? null
        const installmentLimits = { ...(state.paymentSourceInstallmentLimits ?? {}) }
        delete installmentLimits[oldVal]
        installmentLimits[nextVal] = oldLimit
        nextState.paymentSourceInstallmentLimits = installmentLimits
    ...
    case 'SET_PAYMENT_SOURCE_INSTALLMENT_LIMIT': {
      const { source, limit } = action.payload
      if (!state.paymentSources.includes(source)) return state
      const value = limit == null ? null : Math.max(0, Math.floor(Number(limit)) || 0)
      return {
        ...state,
        paymentSourceInstallmentLimits: {
          ...(state.paymentSourceInstallmentLimits ?? {}),
          [source]: value,
        },
      }
    ...
    case 'SET_INSTALLMENT_MIN_AMOUNT': {
      return { ...state, installmentMinAmount: Math.max(0, Math.floor(action.payload)) }
    }
    ...
    case 'LOAD_API_DATA': {
      const p = action.payload
      const nextSources = p.paymentSources ?? state.paymentSources
      const nextModes = p.paymentSourceModes ?? state.paymentSourceModes
      const nextStatementDays = p.paymentSourceStatementDays ?? state.paymentSourceStatementDays
      return {
        ...state,
        ...(p.transactions !== undefined && { transactions: p.transactions }),
        ...(p.spendingPeriods !== undefined && { spendingPeriods: p.spendingPeriods }),
        ...(p.installmentPlans !== undefined && { installmentPlans: p.installmentPlans }),
        ...(p.debts !== undefined && { debts: p.debts }),
        ...(p.receivables !== undefined && { receivables: p.receivables }),
        ...(p.confirmedStatements !== undefined && { confirmedStatements: p.confirmedStatements }),
        ...(p.categories !== undefined && { categories: p.categories }),
        ...(p.categoryKinds !== undefined && { categoryKinds: p.categoryKinds }),
        ...(p.paymentSources !== undefined && { paymentSources: nextSources }),
        ...(p.paymentSourceModes !== undefined && { paymentSourceModes: nextModes }),
        ...(p.paymentSourceStatementDays !== undefined && { paymentSourceStatementDays: nextStatementDays }),
        ...(p.paymentSourceDueDays !== undefined && { paymentSourceDueDays: p.paymentSourceDueDays }),
        ...(p.paymentChannels !== undefined && { paymentChannels: p.paymentChannels }),
        ...(p.installmentMinAmount !== undefined && { installmentMinAmount: p.installmentMinAmount }),
        ...(p.calendarRules !== undefined && { calendarRules: p.calendarRules }),
        bnplProviders: buildBnplProvidersFromConfig(nextSources, nextModes, nextStatementDays),
      }
    }
