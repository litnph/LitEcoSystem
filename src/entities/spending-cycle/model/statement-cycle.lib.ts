...
export function getStatementPeriodForDate(occurredAt: string, statementDay: number): string {
...
export function isInStatementPeriod(occurredAt: string, period: string, statementDay: number): boolean {
...
export function getEffectiveBnplStatementPeriod(t: BnplPeriodSource, statementDay: number): string {
  if (t.bnplDeferredToPeriod) return t.bnplDeferredToPeriod
  return getStatementPeriodForDate(t.occurredAt, statementDay)
}

export function nextStatementPeriod(period: string): string {
  const [ys, ms] = period.split('-')
  const y = Number(ys)
  const m = Number(ms)
