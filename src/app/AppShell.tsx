  ...
  return (
    <button
      ...
      className={`group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition ${
        active
          ? 'bg-[#62492E] text-white shadow-sm'
          : 'text-[#9E8E7C] hover:bg-[#EDE6DC] hover:text-[#2C2215]'
      }`}
    >
      <Icon
        className={`h-4 w-4 shrink-0 ${
          active ? 'text-white' : 'text-[#BFB0A0] group-hover:text-[#5C4A36]'
        }`}
      />
...
export function AppShell() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F9F6F2] text-[#2C2215]">
      ...
      <aside
        className={`fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-[#E4D9CE] bg-[#F5F0E8] transition-transform duration-200 ease-out ${
      ...
      >
        ...
        <main className="flex-1 p-4 md:p-6">
          ...
          {apiLoading ? (
            <div className="flex items-center justify-center gap-3 py-24 text-[#9E8E7C]">
              <svg className="h-5 w-5 animate-spin text-[#7A5E3E]" fill="none" viewBox="0 0 24 24">
          ...
          return (
            <button
              ...
              className={`flex flex-1 flex-col items-center gap-1 py-3 text-[10px] font-medium transition ${
                active ? 'text-[#62492E]' : 'text-[#BFB0A0]'
              }`}
            >
